 ...
 */
size_t get_meta(struct gen_pool_meta *meta, int name)
{
	size_t size = 0;
	switch(name) {
		case HEAP_SIZE:
 ...
 *
 */
void set_meta(struct gen_pool_meta *meta, int name, size_t size)
{
	switch(name) {
		case HEAP_SIZE:
			spin_lock(&meta->lock);
...
size_t gen_pool_largest_free_buf(struct gen_pool *pool) 
{
	...
	list_for_each_entry_rcu(chunk, &pool->chunks, next_chunk) {
		...
		while(next_free_buf < bitmap_size) {
			...
			buf_size > largest_free_buf ? largest_free_buf = buf_size : 1;
